...
with col1:
    st.image("assets/Lambang_Badan_Pusat_Statistik_(BPS)_Indonesia.png", width=120)
with col2:
...
# ============================
    ...
    for col in ["Realisasi APBD", "Realisasi PMA", "Realisasi PMDN", "Belanja Infrastruktur", "Ekspor Luar Negeri", "Produksi Padi"]:
        df[col] = pd.to_numeric(df[col], errors="coerce").astype("Int64")

    for col in ["IPH Mojokerto", "Inflasi Kediri"]:
    ...
    if not df_filtered.empty:
        ...
        ax1.plot(df_filtered.index, df_filtered["Ekspor Luar Negeri"], marker='o')
        st.pyplot(fig1)

        # GRAFIK PADI
        st.subheader("Tren Produksi Padi per Triwulan")
        fig2, ax2 = plt.subplots(figsize=(8, 5))
        ax2.plot(df_filtered.index, df_filtered["Produksi Padi"], marker='s')
        st.pyplot(fig2)

        # GRAFIK IPH VS INFLASI
        st.subheader("Layered Grafik: IPH Mojokerto vs Inflasi Kediri")
        fig, ax1 = plt.subplots(figsize=(8, 5))
        ax1.plot(df_filtered.index, df_filtered["IPH Mojokerto"], marker='o', color="blue")
        ax2 = ax1.twinx()
        ax2.plot(df_filtered.index, df_filtered["Inflasi Kediri"], marker='s', linestyle='--', color="red")
        st.pyplot(fig)
